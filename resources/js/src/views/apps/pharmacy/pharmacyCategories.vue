<template>
	<vx-card>
		...
		<vs-tabs alignment="fixed" :color="tabColor">
			<vs-tab @click="tabColor = 'success'" label="Manage Category">
				<div>
<vx-card>
		<vs-table :data="categories" search stripe pagination max-items="3">
			...
			<vs-tab @click="tabColor = 'success'" label="Add Category">
				<div>
			...
			<div class="vx-row">
				<div class="vx-col sm:w-1/2 w-full mb-4">
					<vs-input class="w-full" icon-pack="feather" icon="icon-plus" label-placeholder="Category Name*"
						v-model="catName" />
				</div>
			</div>
			...
			<div class="vx-row">
				<div class="vx-col w-full">
					...
					<vs-popup background-color="rgba(255,255,255,.6)" :background-color-popup="colorx" class=""
						...
						<p> Are You Sure You Want to Add This Medicine Category?</p><br>
					...
					<vs-button color="warning" type="border" class="mb-2"
						@click="catName = ''; check7 = false;">Reset</vs-button>
				</div>
			</div>
		</vx-card>
